...
/**
 * Base class that both sequential and alternating executions inherit from.
 * Defines functionality that any execution should have.
 */
public abstract class AbstractExecution extends EventResponder implements
        Matchable, Queryable {
    protected boolean isZeroPlus, isOnePlus;
    protected boolean doneOnce = false;

    public boolean isDoneOnce() {
        return doneOnce;
    }

    public void setDoneOnce(boolean doneOnce) {
        this.doneOnce = doneOnce;
    }
    ...
    public void resetChildrenCursor() {
        ...
        if (children != null) {
            for (int i = 0; i < this.children.size(); i++) {
                Class<? extends EventResponder> classChild = children.get(i)
                        .getClass();
