...
public class PolicyVisitor extends PoCoParserBaseVisitor<Void> {
    ...
    public Void visitIre(@NotNull PoCoParser.IreContext ctx) {
        ...
        if (ctx.re(0) != null) {
            ...
            if (isAction) {
                actionresultname = "action" + ireNum++;
                outLine(3, "IRE %s = new IRE(\"action\");", ireName);
                outLine(3, "Action %s = new Action(\"%s\");", actionresultname, reString);
                outLine(3, "%s.Action = %s;" , ireName, actionresultname);
                outLine(3, "%s.Result = null;",ireName);
            }else {
                ...
                actionresultname = "result" + ireNum++;
                outLine(3, "IRE %s = new IRE(\"result\");", ireName);
                outLine(3, "Result %s = new Result(\"%s\");", actionresultname, reString2);
                outLine(3, "%s.Action = null;",ireName);
                outLine(3, "%s.Result = %s;"  ,ireName, actionresultname);
            }
            // Add this ire's action match string to the current Match object
            outLine(3, "%s.setMatchString(%s);", currentMatch, ireName);
